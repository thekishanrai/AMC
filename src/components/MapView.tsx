...
export default function MapView({ initialSpot = null }: { initialSpot?: Spot | null }) {
  ...
  return <div className={`amc-app section-${section}`}><div className="amc-map" ref={containerRef}/>{section==="explore"&&!mapReady&&<div className="amc-map-loading amc-static-map"/>}<TopBar search={search} onSearch={setSearch}/>{section==="explore"&&<>{gate&&!gateDismissed&&<GeoGateBanner city={gate.city} onDismiss={()=>setGateDismissed(true)} onNotify={()=>setGateDismissed(true)}/>}<div className="amc-location-float"><LocationPicker label={locationLabel} confirmed={locationConfirmed} onPick={handlePickCity} onUseGps={handleNearMe} locatingGps={locating}/></div><aside className={`amc-discovery-panel state-${drawerState}`}><button className="amc-drawer-grab" aria-label={`Drawer ${drawerState}`} onClick={()=>{if(!drawerMoved.current)nextDrawer(drawerState==="full"?-1:1);drawerMoved.current=false}} onPointerDown={e=>{drawerStartY.current=e.clientY;drawerMoved.current=false;e.currentTarget.setPointerCapture(e.pointerId)}} onPointerMove={e=>{if(drawerStartY.current!==null&&Math.abs(e.clientY-drawerStartY.current)>12)drawerMoved.current=true}} onPointerUp={e=>{if(drawerStartY.current!==null){const d=e.clientY-drawerStartY.current;if(d< -35)nextDrawer(1);else if(d>35)nextDrawer(-1)}drawerStartY.current=null}}><span/></button><div className="amc-drawer-scroll"><div className="amc-panel-title"><div><p>NEAR {locationLabel.toUpperCase()}</p><h1>{spots.length===0?"Finding places":visible.length+" places better than Monday"} <span>· {maxDistanceKm} km</span></h1><small className="amc-panel-tagline">{visible.length} places to forget Monday exists</small></div></div>{drawerState!=="min"&&<><CategoryChips active={activeCategory} onChange={setActiveCategory} maxDistanceKm={maxDistanceKm} onDistanceChange={setMaxDistanceKm}/><div className="amc-results-label"><span><strong>PLACES BETTER THAN MONDAY</strong><small>within {maxDistanceKm} km</small></span><span>NEAREST FIRST ↓</span></div><div className="amc-card-list" onScroll={e=>{const box=(e.currentTarget as HTMLElement).getBoundingClientRect();const cards=[...e.currentTarget.querySelectorAll<HTMLElement>("[data-spot-id]")];const best=cards.sort((a,b)=>Math.abs(a.getBoundingClientRect().left-box.left)-Math.abs(b.getBoundingClientRect().left-box.left))[0];const spot=visible.find(s=>s.id===best?.dataset.spotId);if(spot)focusSpot(spot)}}>{spots.length===0?[0,1].map(i=><div key={i} className="amc-spot-card amc-card-skeleton"><i/><b/><span/></div>):visible.map(s=><SpotCard key={s.id} spot={s} distance={distanceFor(s)} slug={slugById.get(s.id)} onFocus={()=>focusSpot(s)} onOpen={()=>openSpot(s)}/>)}</div></>}</div></aside></>}{section==="surprise"&&<SurpriseMe spots={spots} origin={initialView?.center??null} saved={savedIds} onSave={save} slugFor={s=>slugById.get(s.id)}/>} {section==="account"&&<AccountView spots={spots} saved={savedIds} distanceFor={distanceFor} slugFor={s=>slugById.get(s.id)} onFocus={focusSpot} onOpen={openSpot}/>} {selectedSpot&&<SpotSheet spot={selectedSpot} onClose={closeSheet}/>} {!selectedSpot&&<BottomNav active={section} onChange={setSection}/>}</div>;
}
