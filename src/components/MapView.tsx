...
export default function MapView({ initialSpot = null }: { initialSpot?: Spot | null }) {
  ...
  const[drawerState,setDrawerState]=useState<"min"|"half"|"full">("half");const drawerStartY=useRef<number|null>(null),drawerStartX=useRef(0),drawerStartHeight=useRef(0),drawerStartTime=useRef(0),drawerLastY=useRef(0),drawerLastTime=useRef(0),drawerStartScroll=useRef(0),drawerMoved=useRef(false),drawerGesture=useRef<"pending"|"drawer"|"content">("pending");
  ...
  return <div className={`amc-app section-${section}`}><div className="amc-map" ref={containerRef}/>{section==="explore"&&!mapReady&&<div className="amc-map-loading amc-static-map"/>}<TopBar search={search} onSearch={value=>{setSearch(value);setCardLimit(12)}}/>{section==="explore"&&<>{gate&&!gateDismissed&&<GeoGateBanner city={gate.city} onDismiss={()=>setGateDismissed(true)} onNotify={()=>setGateDismissed(true)}/>}<div className="amc-location-float"><LocationPicker label={locationLabel} confirmed={locationConfirmed} onPick={handlePickCity} onUseGps={handleNearMe} locatingGps={locating}/></div><aside className={`amc-discovery-panel state-${drawerState}`} onClickCapture={e=>{if(drawerMoved.current){e.preventDefault();e.stopPropagation();drawerMoved.current=false}}} onPointerDown={e=>{if(e.pointerType==="mouse"&&e.button!==0)return;drawerStartY.current=e.clientY;drawerStartX.current=e.clientX;drawerStartTime.current=performance.now();drawerLastY.current=e.clientY;drawerLastTime.current=drawerStartTime.current;drawerMoved.current=false;drawerGesture.current="pending"}} onPointerMove={e=>{if(drawerStartY.current===null)return;const panel=e.currentTarget,dy=e.clientY-drawerStartY.current,dx=e.clientX-drawerStartX.current,scroll=panel.querySelector<HTMLElement>(".amc-drawer-scroll");if(drawerGesture.current==="pending"&&Math.max(Math.abs(dx),Math.abs(dy))>7){
  drawerStartHeight.current=panel.getBoundingClientRect().height;
  drawerStartScroll.current=scroll?.scrollTop??0;
  if(Math.abs(dx)>Math.abs(dy)){drawerGesture.current="content";drawerStartY.current=null;return}
  const startedOnCards=Boolean((e.target as HTMLElement).closest(".amc-card-list"));
  const scrollAtEnd=Boolean(scroll&&scroll.scrollTop+scroll.clientHeight>=scroll.scrollHeight-2);
  // In the default state an upward pull on the cards browses the inner sheet
  // first. Only a fresh pull after its scroll reaches the end expands it.
  const scrollAtTop=(scroll?.scrollTop??0)<=2;
  const shouldScrollUp=dy<0&&((drawerState==="half"&&startedOnCards&&!scrollAtEnd)||drawerState==="full");
  const shouldScrollDown=dy>0&&startedOnCards&&!scrollAtTop;
  if(shouldScrollUp||shouldScrollDown){drawerGesture.current="content";drawerMoved.current=true;panel.setPointerCapture(e.pointerId)}
  else{drawerGesture.current="drawer";drawerMoved.current=true;panel.setPointerCapture(e.pointerId);panel.classList.add("is-dragging")}
}
if(drawerGesture.current==="content"){if(scroll)scroll.scrollTop=Math.max(0,drawerStartScroll.current-dy);return}if(drawerGesture.current!=="drawer")return;const min=169,max=window.innerHeight-66;panel.style.setProperty("height",`${Math.max(min,Math.min(max,drawerStartHeight.current-dy))}px`);drawerLastY.current=e.clientY;drawerLastTime.current=performance.now()}} onPointerUp={e=>{const panel=e.currentTarget;if(drawerStartY.current!==null&&drawerGesture.current==="drawer"){const d=e.clientY-drawerStartY.current,elapsed=Math.max(1,performance.now()-drawerLastTime.current),velocity=(e.clientY-drawerLastY.current)/elapsed;if(d< -30||velocity<-.35)nextDrawer(1);else if(d>30||velocity>.35)nextDrawer(-1)}drawerStartY.current=null;drawerGesture.current="pending";panel.classList.remove("is-dragging");panel.style.removeProperty("height")}} onPointerCancel={e=>{drawerStartY.current=null;drawerGesture.current="pending";e.currentTarget.classList.remove("is-dragging");e.currentTarget.style.removeProperty("height")}}><button className="amc-drawer-grab" data-drawer-handle aria-label={`Drawer ${drawerState}. Drag to resize`} onClick={()=>{if(!drawerMoved.current)nextDrawer(drawerState==="full"?-1:1);drawerMoved.current=false}}><span/></button><div className="amc-drawer-scroll"><div className="amc-panel-title"><div><p>NEAR {locationLabel.toUpperCase()}</p><h1>{spots.length===0?"Finding places":visible.length+" places to forget Monday exists"}</h1></div></div>{drawerState!=="min"&&<><CategoryChips active={activeCategory} onChange={value=>{setActiveCategory(value);setCardLimit(12)}} maxDistanceKm={maxDistanceKm} onDistanceChange={value=>{setMaxDistanceKm(value);setCardLimit(12)}}/><div className="amc-results-label"><span><strong>PLACES BETTER THAN MONDAY</strong><small>within {maxDistanceKm} km</small></span></div><div className="amc-card-list" onScroll={e=>{const rail=e.currentTarget;const box=rail.getBoundingClientRect();const cards=[...rail.querySelectorAll<HTMLElement>("[data-spot-id]")];const best=cards.sort((a,b)=>Math.abs(a.getBoundingClientRect().left-box.left)-Math.abs(b.getBoundingClientRect().left-box.left))[0];const spot=visible.find(s=>s.id===best?.dataset.spotId);if(spot)focusSpot(spot);if(rail.scrollLeft+rail.clientWidth>=rail.scrollWidth-460)setCardLimit(limit=>Math.min(visible.length,limit+12))}}>{spots.length===0?[0,1].map(i=><div key={i} className="amc-spot-card amc-card-skeleton"><i/><b/><span/></div>):visible.slice(0,cardLimit).map(s=><SpotCard key={s.id} spot={s} distance={distanceFor(s)} slug={slugById.get(s.id)} onFocus={()=>focusSpot(s)} onOpen={()=>openSpot(s)}/>)}</div></>}</div></aside></>}{section==="surprise"&&<SurpriseMe spots={spots} origin={initialView?.center??null} saved={savedIds} onSave={save} slugFor={s=>slugById.get(s.id)}/>} {section==="account"&&<AccountView spots={spots} saved={savedIds} distanceFor={distanceFor} slugFor={s=>slugById.get(s.id)} onFocus={focusSpot} onOpen={openSpot}/>} {selectedSpot&&<SpotSheet spot={selectedSpot} onClose={closeSheet}/>} {!selectedSpot&&<BottomNav active={section} onChange={setSection}/>}</div>;
}
